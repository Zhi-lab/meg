...
def map_generation():
    ...
    for i in range(0, cf.storeNum):
        store_position.append([2 * random.randint(0, cf.blockEachLine -1), 2 * random.randint(0, cf.blockEachLine -1)])
...
def subjectMapGeneration(blockEachLine):
    subjectMap = []
    for i in range(0,blockEachLine * 2):
        currentRow = []
        for j in range(0,blockEachLine * 2):
            currentRow.append(0)
        subjectMap.append(currentRow)
    return subjectMap